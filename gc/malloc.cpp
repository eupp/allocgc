...
// TODO somewhere we need to deallocate free space
	...
	for (SegregatedList * sl = &segregated_storage[i]; i < SegregatedStorageSize; i++, sl = &segregated_storage[i]) {
		...
		if (!sle1) { continue; }
		int descr_i_1 = -1, descr_i_2 = SSEDescrCount - 1;
		...
		int sf_i = ObjectsPerPage - 1, ff_i = 0, last_occupated_object = -1;
		...
		while (true) {
			while (!sf_mark) {
				if (sf_i == -1) {
					clear_page_flags(d2);
					if (descrs_met) {
						int last_alive_object_on_page = max(last_occupated_object, ff_i);
						d2->free = last_alive_object_on_page == ObjectsPerPage - 1
						           ? NULL
						           : (void *)((size_t)d2->page + d2->obj_size * (last_alive_object_on_page + 1));
						goto descrs_met_flag;
					} else {
						assert(d2 != NULL);
						d2->free = last_occupated_object == ObjectsPerPage - 1
						           ? NULL
						           : (void *)((size_t)d2->page + d2->obj_size * (last_occupated_object + 1));
					}
					descrs_met = descrs_met || get_prev_descr(sle2, &descr_i_2, d2, d1);
					last_occupated_object = -1, sf_mark = false, sf_pin = false, sf_i = ObjectsPerPage - 1;
				}
				...
				if (descrs_met && ff_i >= sf_i) {
					assert(d1 == d2);
					clear_page_flags(d2);
					d2->free = (void *)((size_t)d2->page + d2->obj_size * (max(ff_i, last_occupated_object)));
					goto descrs_met_flag;
				}
				if (sf_mark) {
			...
			if (ff_i >= sf_i && descrs_met) {
				assert(d1 == d2);
				clear_page_flags(d1);
				d1->free = (void *)((size_t)d1->page + d1->obj_size * (max(ff_i, last_occupated_object)));
				goto descrs_met_flag;
			}
