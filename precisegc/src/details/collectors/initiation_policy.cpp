...
growth_based_policy::growth_based_policy(size_t start_heap_size, double freespace_divisor, size_t max_heap_size)
    : m_heap_size(start_heap_size)
    , m_freespace_divisor(freespace_divisor)
    , m_max_heap_size(max_heap_size)
{ }
                                           ...
                                           /*,const initiation_point_data& ipd*/)
{
    ...
    if (ipt == initiation_point_type::HEAP_EXPANSION) {
        ...
        if (gain > state.heap_size / m_freespace_divisor) {
            launcher->gc(gc_phase::COLLECT);
            size_t new_size = launcher->state().heap_size + ipd.alloc_size();
            if (new_size > m_max_heap_size) {
                throw gc_bad_alloc();
            }
        } else if (gain > state.heap_size / (MARKING_FACTOR * m_freespace_divisor)) {
            return launcher->gc(gc_phase::MARK);
        }
    }
     */
...
size_t growth_based_policy::max_heap_size() const
{
    return m_max_heap_size;
}

double growth_based_policy::freespace_divisor() const
{
    return m_freespace_divisor;
}
