                                                  ...
                                                  const thread_descriptor& main_thrd_descr)
{
    if (options.algo == gc_algo::SERIAL) {
        if (options.compacting == gc_compacting::DISABLED) {
            return utils::make_unique<gc_serial>(options.threads_available, main_thrd_descr);
        } else if (options.compacting == gc_compacting::ENABLED) {
            return utils::make_unique<gc_serial>(options.threads_available, main_thrd_descr);
        }
    } else if (options.algo == gc_algo::INCREMENTAL) {
        if (options.compacting == gc_compacting::DISABLED) {
            return utils::make_unique<gc_incremental>(options.threads_available, main_thrd_descr);
        } else if (options.compacting == gc_compacting::ENABLED) {
            return utils::make_unique<gc_incremental>(options.threads_available, main_thrd_descr);
        }
    }
