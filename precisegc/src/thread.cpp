...
// function witch will be called
// like a signal handler for gc
void sig_func(int sig)
{
    if (sig == 16) {
        fprintf(stderr, "sig_func : thread catch signal SIGSGV\n");
        write(1, "Caught signal 16\n", 17);
    }
}
...
void suspend_threads(void)
{
    ...
    while (handler) {
        if (!thread_in_safepoint(handler)) {
            pthread_kill(handler->thread, 16);
        }
