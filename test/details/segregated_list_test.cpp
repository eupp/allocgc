...
{
    unique_ptr<segregated_list_element> sle(new segregated_list_element());
    for (int i = 0; i < PAGES_PER_SEGREGATED_STORAGE_ELEMENT; ++i) {
        ASSERT_TRUE(sle->is_memory_available(OBJ_SIZE));
        auto alloc_res = sle->allocate(OBJ_SIZE);
    ...
    ASSERT_FALSE(sle->is_memory_available(OBJ_SIZE));
    ASSERT_FALSE(sle->is_memory_available(1));
}
