...
TEST_F(space_base_policy_test, test_check)
{
    ...
    ASSERT_FALSE(policy.check(stat, initation_point_type::HEAP_GROWTH));

    stat.heap_size = policy.threshold() * policy.heap_size() + 1;
    ASSERT_TRUE(policy.check(stat, initation_point_type::HEAP_GROWTH));
}

TEST_F(space_base_policy_test, test_update)
{
    stat.heap_size = policy.heap_size();
    policy.update(stat, initation_point_type::HEAP_GROWTH);
    ...
    ASSERT_THROW(policy.update(stat, initation_point_type::HEAP_GROWTH), gc_bad_alloc);
}
...
TEST_F(incremental_space_base_policy_test, test_check)
{
    ...
    ASSERT_EQ(gc_phase::IDLING, policy.check(stat, initation_point_type::HEAP_GROWTH));

    stat.heap_size = policy.heap_size() * policy.marking_threshold() + 1;
    ASSERT_EQ(gc_phase::MARKING, policy.check(stat, initation_point_type::HEAP_GROWTH));

    stat.heap_size = policy.heap_size() * policy.sweeping_threshold() + 1;
    ASSERT_EQ(gc_phase::SWEEPING, policy.check(stat, initation_point_type::HEAP_GROWTH));
}